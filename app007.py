...
# -----------------------------------------------------------------------------
# [섹션 3] 개별 종목 스크리닝 (단기 목표가 및 손절가 추가)
# -----------------------------------------------------------------------------
...
if not df_universe.empty:
    ...
    # [추가] 실전 단타 가이드라인 계산
    filtered_df['단기_목표가'] = (filtered_df['현재가'] * 1.03).astype(int)  # +3% 수익 목표가
    filtered_df['손절가'] = (filtered_df['현재가'] * 0.98).astype(int)      # -2% 기계적 손절가
    filtered_df['상한가_여력'] = (30.0 - filtered_df['등락률']).round(2)      # 상한가(30%)까지 남은 퍼센트

    top_30 = filtered_df.sort_values(by='우량주_매력도_점수', ascending=False).head(30)

    # 출력용 데이터프레임 구성 (새로운 열 추가)
...
with st.expander("📖 스캐너 작동 로직 및 실전 단타 가이드 (클릭하여 펼치기)", expanded=True):
    ...
    * **주도주 스크리닝**: API를 두 번 호출하여 '중소형주(1~8만 원)'와 '대형 우량주(8~200만 원)'의 데이터를 모은 뒤, 오직 **'당일 누적 거래대금(몰린 돈)'** 기준으로 랭킹을 다시 매겨 진짜 주도주를 뽑아냅니다.
    * **매력도 점수 산출**: `(등락률 × 1.5) + (log(거래대금) × 2.5)` 공식을 적용하여 상승 탄력과 돈의 힘이 완벽히 맞아떨어지는 종목을 찾습니다.

    ### 🎯 2. 실전 단타 가이드라인 활용법
    * **단기 목표가 (+3%)**: 해당 종목 진입 시, 감정에 휩쓸리지 않고 기계적으로 분할 매도를 시작해야 할 1차 익절 라인입니다.
    * **손절가 (-2%)**: 단타는 대응이 생명입니다. 진입 후 이 가격을 이탈하면 뒤도 돌아보지 말고 손절해야 하는 마지노선입니다.
    * **상한가까지 여력**: 현재가에서 상한가(30%)까지 얼마나 남았는지를 보여줍니다. 이 수치가 너무 작다면(예: 3% 이하) 먹을 구간이 적고 리스크가 크므로 진입을 피하는 것이 좋습니다.
